...
pushd "%~dp0%WCROOT%" && (
  ...
  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%TACKLELIB_SCRIPTS.GIT.ORIGIN%%"
  ) || call :CMD git remote add origin "%%TACKLELIB_SCRIPTS.GIT.ORIGIN%%" || ( popd & goto EXIT )
