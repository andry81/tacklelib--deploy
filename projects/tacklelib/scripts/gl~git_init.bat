...
pushd "%~dp0%WCROOT%" && (
  rem reinit git svn
  call :GIT_SVN_INIT "%%TACKLELIB_SCRIPTS.SVN.REPOROOT%%" --stdlayout || ( popd & goto EXIT )

  call :CMD git config user.name "%%GIT3.USER%%" || ( popd & goto EXIT )
  call :CMD git config user.email "%%GIT3.EMAIL%%" || ( popd & goto EXIT )

  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%TACKLELIB_SCRIPTS.GIT3.ORIGIN%%"
  ) || call :CMD git remote add origin "%%TACKLELIB_SCRIPTS.GIT3.ORIGIN%%" || ( popd & goto EXIT )
